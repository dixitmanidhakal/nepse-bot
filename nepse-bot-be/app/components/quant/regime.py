...
def detect_regime_numpy_fallback(
    prices_series: pd.Series,
    n_states: int = 3,
    lookback: int = 252,
) -> dict:
    """
    Pure-numpy/scipy regime detection fallback used when hmmlearn is absent.

    Instead of a hidden Markov model, this implementation:
      1. Computes log-returns and a rolling 20-day realised volatility.
      2. Assigns each observation to one of ``n_states`` regimes using
         percentile-based thresholds on the return series.
      3. Reads the *last* observation to determine the current regime and
         builds soft-probability estimates from a Gaussian kernel.

    The result dict mirrors the shape returned by ``detect_regime_from_prices``
    so the route layer can treat both interchangeably.
    """
    returns = prices_series.pct_change().dropna().values
    ...
    for i in range(vol_window, len(returns)):
        vol[i] = float(np.std(returns[i - vol_window : i], ddof=1))

    # Use only rows where volatility is valid
    valid_mask = vol > 0
    valid_returns = returns[valid_mask]

    if len(valid_returns) < n_states:
        return {
            "regime": "neutral",
            "probabilities": {"bull": 0.33, "neutral": 0.34, "bear": 0.33},
            "confidence": 0.34,
            "exposure_multiplier": 0.5,
            "method": "numpy_fallback_insufficient_data",
        }

    # Percentile-based regime thresholds
    pct_lo = 100.0 / n_states
    pct_hi = 100.0 - pct_lo
    thresh_lo = float(np.percentile(valid_returns, pct_lo))
    thresh_hi = float(np.percentile(valid_returns, pct_hi))

    last_return = float(returns[-1])
    last_vol = float(vol[-1]) if vol[-1] > 0 else float(np.mean(vol[valid_mask]))

    # Soft regime probabilities via Gaussian kernels centred on percentiles
    sigma = max(float(np.std(valid_returns, ddof=1)), 1e-8)

    def gauss_kernel(x: float, mu: float) -> float:
        return float(np.exp(-0.5 * ((x - mu) / sigma) ** 2))

    if n_states == 2:
        p_bull = gauss_kernel(last_return, thresh_hi)
        p_bear = gauss_kernel(last_return, thresh_lo)
        total = p_bull + p_bear + 1e-12
        probs = {"bull": p_bull / total, "bear": p_bear / total}
        regime = max(probs, key=probs.__getitem__)  # type: ignore[arg-type]
    else:
        p_bull = gauss_kernel(last_return, thresh_hi)
        p_neutral = gauss_kernel(last_return, (thresh_lo + thresh_hi) / 2)
        p_bear = gauss_kernel(last_return, thresh_lo)
        total = p_bull + p_neutral + p_bear + 1e-12
        probs = {
            "bull": p_bull / total,
        ...
        if n_states > 3:
            # Add extra states by splitting neutral
            step = 1.0 / n_states
            for k in range(1, n_states - 2):
                label = f"neutral_{k}"
                probs[label] = probs["neutral"] * step
            probs["neutral"] *= step
        regime_name = max(probs, key=probs.__getitem__)  # type: ignore[arg-type]
        regime = regime_name

    confidence = float(probs.get(regime, 0.34))
    p_bull_val = float(probs.get("bull", 0.0))
    p_neutral_val = float(probs.get("neutral", 0.0))
    exposure = p_bull_val * 1.0 + p_neutral_val * 0.5

    return {
        "regime": regime,
        "probabilities": {k: round(v, 4) for k, v in probs.items()},
        "confidence": round(confidence, 4),
        "exposure_multiplier": round(float(np.clip(exposure, 0.0, 1.0)), 4),
        "method": "numpy_percentile_fallback",
    }
